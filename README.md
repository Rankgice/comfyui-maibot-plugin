...
- **自动参数替换**: 自动替换工作流中的 `${prompt}` (提示词) 和 `${seed}` (随机种子)。
...
2. **添加占位符**:
    - 找到输入提示词的节点（例如 `CLIPTextEncode`），将其 `text` 字段的值修改为 `"${prompt}"`。
    - 找到控制随机种子的节点（例如 `KSampler`），将其 `seed` 字段的值修改为 `"${seed}"`。
...
├── workflow/           # 存放工作流文件的目录
│   └── text_to_image_z_image_turbo_api.json # 默认工作流文件
└── README.md           # 项目说明文档
```
